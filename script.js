document.addEventListener('DOMContentLoaded', function() {
    ...
    function distributeItems(items) {
        ...
        if (isMobile) {
            console.log(`Distributing to mobile: total ${items.length} items`);
            
            // Each container gets about 10 items
            const itemsPerContainer = Math.ceil(items.length / 4);
            ...
            mobileContainers.forEach((container, index) => {
                if (container) {
                    container.innerHTML = '';
                    const start = index * itemsPerContainer;
                    const end = Math.min((index + 1) * itemsPerContainer, items.length);
                    
                    console.log(`Container ${index}: assigning ${end - start} items`);
            ...
            // Normal mobile distribution (backup method)
